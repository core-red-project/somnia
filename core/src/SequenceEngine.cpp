...
NoteEvent SequenceEngine::nextEvent(StreamState& state, float timeStep, bool useLydian,
    ...
    static constexpr std::array<uint16_t, 3> subdivisions = {100, 200, 400};

    float t = state.time;

    // Sample pitch index using multi-octave FBM
    float pitchSample = m_patternEngine.getFbmSample(t, seed, 2);
    float scaledPitch = std::clamp(pitchSample * 100.0f, 0.0f, 100.0f);
    ...
    float scaledRhythm = std::clamp(rhythmSample * static_cast<float>(subdivisions.size()), 0.0f,
                                    static_cast<float>(subdivisions.size() - 1));
