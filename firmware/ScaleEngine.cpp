...
Note ScaleEngine::getNote(size_t index, bool useLydian) const {
    std_compat::span<const int> intervals =
        useLydian ? std_compat::span<const int>(m_lydianIntervals)
                  : std_compat::span<const int>(m_minorPentatonicIntervals);
    ...
    static constexpr std_compat::string_view notesTable[5][12] = {
    ...
    std_compat::string_view name = notesTable[octaveIndex][noteIdx];
